  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        <FilterLink href="/tasks" label="すべて" active={!params.status} />
        <FilterLink
        ...
        />
        <FilterLink
          href="/tasks?status=reviewed"
          label="レビュー済"
          active={params.status === "reviewed"}
        />
        <FilterLink
          href="/tasks?status=pending_ceo_approval"
          label="社長承認待ち"
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-center gap-2">
          <Button variant="outline" size="sm" asChild disabled={page <= 1}>
            <Link
              href={`/tasks?${new URLSearchParams({ ...(params.status ? { status: params.status } : {}), page: String(page - 1) })}`}
            >
              前へ
            </Link>
          </Button>
