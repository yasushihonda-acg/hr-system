...
intentStatsRoutes.get(
  ...
  async (c) => {
    ...
    let query = collections.intentRecords
      .where("createdAt", ">=", Timestamp.fromDate(from))
      .where("createdAt", "<=", Timestamp.fromDate(to));

    if (method !== "all") {
      query = query.where("classificationMethod", "==", method);
    }
...
intentStatsRoutes.get("/override-patterns", async (c) => {
  const overrideSnap = await collections.intentRecords.where("isManualOverride", "==", true).get();
  ...
  const fetchPromises = [...allMsgIds].map(async (id) => {
