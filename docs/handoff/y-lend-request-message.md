...
- **拒否されるツール**: 従業員更新・作成（403 が返る仕様）
- **提供していない機能**: 給与明細参照（本 MCP では完全に未提供）
...
**ポイント**:
- 3 は 403 が返ることが「正しい動作」です。readonly 強制が効いている証拠になるので、必ず試してください
...
- 3 が成功してしまう → **即連絡**（readonly 強制が効いていない異常、セキュリティインシデント）
