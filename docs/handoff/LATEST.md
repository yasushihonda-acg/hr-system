...
**ブランチ**: `main`（最新コミット: `d68bbfb` — 全変更 push 済み、未プッシュなし）

---

## 現在のフェーズ

**Phase 2 — LINE 画像メッセージ対応 + GCS Uniform Bucket-Level Access 修正完了**

LINE グループチャット収集・表示機能（PR #113）に加え、画像メッセージの Cloud Storage 保存・表示（PR #119）、GCS Uniform Bucket-Level Access 互換性修正 + バックフィルスクリプト追加（PR #120）を実施。
CI (Deploy to Cloud Run) は最新コミット `d68bbfb` 後に成功・デプロイ完了。
**積み残しタスクなし。**
...
### fix(worker): GCS Uniform Bucket-Level Access との互換性修正 + バックフィルスクリプト追加 (d68bbfb, PR #120)
- `apps/worker/src/lib/storage.ts`: `public: true` を削除（Uniform Bucket-Level Access 有効時にオブジェクトレベル ACL 不可。公開アクセスはバケットレベルの allUsers ポリシーで担保）
- `apps/worker/src/scripts/backfill-line-media.ts`: `contentUrl` 未設定のメディアメッセージを LINE Content API から再取得し Cloud Storage に保存するバックフィルスクリプト追加

### feat: LINE 画像メッセージの取得・Cloud Storage 保存・表示対応 (0b36c27, PR #119)
- `apps/worker/src/lib/line-api.ts`: LINE Content API (MessagingApiBlobClient) で画像バイナリ取得
- `apps/worker/src/lib/storage.ts`: Cloud Storage (`gs://hr-system-487809-line-media/`) へアップロード
- `packages/db/src/types.ts`: `LineMessage` に `contentUrl` フィールド追加
- `apps/api/src/routes/line-messages.ts`: レスポンスに `contentUrl` を含める
- `apps/web/.../line-message-card.tsx`: 画像サムネイル表示、動画/音声/ファイルはリンク表示

### fix(web): LINE メッセージカードの line-clamp-3 を除去して全文表示 (dc3e594, PR #118)
- `apps/web/.../line-message-card.tsx`: テキスト省略クラス `line-clamp-3` を削除し全文表示

### fix(web): toLocaleString による hydration mismatch を修正 (7acf646, PR #115)
- `apps/web/src/lib/utils.ts`: 共通 `formatDateTime` / `formatDate` ユーティリティを追加（`timeZone: Asia/Tokyo` 固定）
- 複数コンポーネントで toLocaleString 直接呼び出しを共通ユーティリティに統一し SSR/CSR 不一致を解消
