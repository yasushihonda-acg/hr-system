...
**ブランチ**: `main`（最新コミット: `e997682` — fix: 担当者・期限の保存後にUIが即時反映されない問題を修正 (#262)）
...
タスク詳細に期限(deadline)フィールド追加（PR #249）、期限の設定・編集 UI 追加（PR #250）、全メッセージ種別（Google Chat / LINE / 手動タスク）で担当者・期限インライン編集対応（PR #252）。インライン編集の外側クリックキャンセル + 手動タスク UI 統一（PR #256）。担当者フィールドをコンボ入力（リスト選択+直入力）に変更（PR #257）。担当者ドロップダウンのキーボードナビ + 期限カレンダーピッカー追加（PR #259）。/api/admin/users の Next.js API route 追加（PR #260）。担当者・期限の保存後 UI 即時反映修正（PR #262）。
...
| **—** | **fix: インライン編集の外側クリックキャンセル + 手動タスクUI統一 (#256)** | **main (2e5d2be)** | **完了** |
| **—** | **feat: 担当者フィールドをコンボ入力（リスト選択+直入力）にする (#257)** | **main (0ad73a7)** | **完了** |
| **—** | **feat: 担当者ドロップダウンのキーボードナビ + 期限カレンダーピッカー (#259)** | **main (e484b92)** | **完了** |
| **—** | **fix: /api/admin/users の Next.js API route を追加（担当者候補取得の404修正） (#260)** | **main (49a7270)** | **完了** |
| **—** | **fix: 担当者・期限の保存後にUIが即時反映されない問題を修正 (#262)** | **main (e997682)** | **完了** |

---

## 直近の変更（最新5件）

### fix: 担当者・期限の保存後にUIが即時反映されない問題を修正 (#262) (e997682)
- 担当者・期限をインライン編集で保存した後、サーバー再フェッチなしに UI が即時反映されなかった問題を修正
- CI: Deploy to Cloud Run — success

### fix: /api/admin/users の Next.js API route を追加（担当者候補取得の404修正） (#260) (49a7270)
- 担当者ドロップダウンが候補を取得する `/api/admin/users` エンドポイントが 404 になっていた問題を修正
- Next.js API Route (`apps/web/src/app/api/admin/users/route.ts`) を新設し、API サーバーへのプロキシとして機能させた

### feat: 担当者ドロップダウンのキーボードナビ + 期限カレンダーピッカー (#259) (e484b92)
- 担当者ドロップダウンにキーボードナビゲーション（上下矢印 + Enter + Escape）を追加
- 期限入力をテキスト入力からカレンダーピッカー（日付選択 UI）に変更

### feat: 担当者フィールドをコンボ入力（リスト選択+直入力）にする (#257) (0ad73a7)
- 担当者入力をドロップダウン選択 + 自由テキスト直入力の両方に対応するコンボ入力に変更

### fix: インライン編集の外側クリックキャンセル + 手動タスクUI統一 (#256) (2e5d2be)
- インライン編集中に外側をクリックすると編集をキャンセルする動作を追加
- 手動タスクの UI を他メッセージ種別と統一
