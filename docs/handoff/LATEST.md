...
**ブランチ**: `main`（最新コミット: `49a7270` — fix: /api/admin/users の Next.js API route を追加（担当者候補取得の404修正） (#260)）
...
タスク詳細に期限(deadline)フィールド追加（PR #249）、期限の設定・編集 UI 追加（PR #250）、全メッセージ種別（Google Chat / LINE / 手動タスク）で担当者・期限インライン編集対応（PR #252）。インライン編集の外側クリックキャンセル + 手動タスク UI 統一（PR #256）。担当者フィールドをコンボ入力（リスト選択+直入力）に変更（PR #257）。担当者ドロップダウンのキーボードナビ + 期限カレンダーピッカー追加（PR #259）。/api/admin/users の Next.js API route 追加（PR #260）。
