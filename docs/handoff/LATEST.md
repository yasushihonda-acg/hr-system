...
**ブランチ**: `main`（最新コミット: `a12c96e` — ui(web): サイドバーナビ再設計 — 可読性・アクティブ状態の改善 (#178)）
...
- CI 完了・Cloud Run デプロイ済み

### fix(web): text-[11px] をタイプスケール準拠の text-xs(12px) に統一 (7d5efd6, PR #177)
- カスタム px 値を Tailwind タイプスケール準拠クラスに統一

### feat: 黄金比ベースのタイポグラフィ・フォントサイズ調整 (cead93e, PR #176)
- 黄金比に基づくタイポグラフィスケールを導入してフォントサイズを整理
