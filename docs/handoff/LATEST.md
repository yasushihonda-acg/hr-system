...
**ブランチ**: `main`（最新コミット: `7acf646` — 全変更 push 済み、未プッシュなし）

---

## 現在のフェーズ

**Phase 2 — LINE グループチャット収集・表示機能 (PR #113) + hydration mismatch 修正完了**

LINE グループチャット収集・表示機能（Webhook受信・Firestore保存・Web表示タブ切替）を追加（PR #113）。
その後ルーティングバグ（`53a79a2`）、toLocaleString による hydration mismatch（`7acf646` / PR #115）を修正。
CI (Deploy to Cloud Run) は最新コミット `7acf646` 後に成功・デプロイ完了。
**積み残しタスクなし。**
...
- `response-status-control.tsx`, `line-message-card.tsx`, `message-card.tsx`, `table-view.tsx`, `sync-button.tsx`: toLocaleString 直接呼び出しを共通ユーティリティに統一し SSR/CSR 不一致を解消
