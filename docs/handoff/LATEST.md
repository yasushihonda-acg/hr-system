...
**ブランチ**: `main`（最新コミット: `fd7a267` — ci: Build ステップを CI に追加（本番ビルドエラーの早期検出））

---

## 現在のフェーズ

**Phase 6 — タスクボード詳細パネル + CI Build ステップ追加**

タスクボードに詳細パネルを追加（ページ遷移なし、#213）し、本番ビルドエラーも修正（#219）。
CI に `pnpm build` ステップを追加し本番ビルドエラーを PR 段階で検出できるよう改善（fd7a267）。

**CI 失敗中（要対応）**: Firestore Emulator 統合テストが CI 環境で ADC 認証エラーにより失敗。
- `dedup.integration.test.js` / `line-webhook.integration.test.js` / `process-message.integration.test.js` が対象
- Root cause: `Error: Could not load the default credentials`（Emulator 起動が CI で設定されていない）
- 直前の Deploy（#219）は成功済み。新 CI Build ステップが統合テストを巻き込んで失敗している。
...
- **注意**: この CI ラン自体は Firestore Emulator 統合テストの ADC 認証エラーで失敗中
...
**CI 失敗中（P0 要対応）: Firestore Emulator 統合テストが CI で ADC 認証エラー**

1. **[P0] CI 統合テスト修正** — `dedup`/`line-webhook`/`process-message` の統合テストが CI で失敗中。Firestore Emulator を CI で起動する設定が必要。`.github/workflows/ci.yml` に `firebase emulators:exec` か Emulator 起動ステップを追加する。
2. **[P2] 未追跡ファイルの整理** — ルートに `help-*.png` 8件と `firestore-debug.log` が残存。画像は `apps/web/public/screenshots/help/` への移動または削除を検討。`firestore-debug.log` は `.gitignore` に追加推奨。
3. **[P2] #200**: Chat 同期操作の権限を admin に限定する（未対応）
