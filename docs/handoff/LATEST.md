...
| # | タイトル | ラベル | 状態 |
|---|---------|--------|------|
| #361 | feat: Chat同期エラー時のグローバルアラートバナー | enhancement, P1 | 未実装（ヘッダーのドットインジケーターはあるが、全ページ赤バナーは未対応） |
| #360 | sync API の GET エンドポイント権限チェック + PII 混入防止 | enhancement, P2 | 未実装（GET全ロール許可・errorMessage未サニタイズのまま） |
