...
**ブランチ**: `main`（最新コミット: `911683d` — feat: Google Chat 受信箱にスペース名を表示（LINE 同様）(#247)）
...
ダッシュボード stats API のキャッシュ追加（PR #242）、未対応フィルター（5000件超）の高速化（Firestore ページネーション）（PR #240）、受信箱フィルター切替のパフォーマンス改善、CI 自動デプロイ改善を実施（PR #237/#238）。

**未コミット変更**: なし（クリーン）

**CI**: Deploy to Cloud Run (#242) — success（確認済み）
